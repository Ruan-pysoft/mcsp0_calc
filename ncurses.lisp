...
(defn nc.wrefresh (win)
  (nc.call "refresh" int ptr win)
)
...
(defn nc.getnstr (n) (nc.wgetnstr (nc.stdscr) n))
...
(defn nc.getstr () (nc.wgetstr (nc.stdscr)))

(defn nc.newwin (nlines ncols begin_y begin_x)
  (nc.call "newwin" ptr int nlines int ncols int begin_y int begin_x)
)
(defn nc.delwin (win)
  (nc.call "delwin" int ptr win)
)
(defn nc.mvwin (win y x)
  (nc.call "mvwin" int ptr win int y int x)
)
(defn nc.subwin (orig nlines ncols begin_y begin_x)
  (nc.call "subwin" ptr ptr orig int nlines int ncols int begin_y int begin_x)
)
(defn nc.derwin (orig nlines ncols begin_y begin_x)
  (nc.call "derwin" ptr ptr orig int nlines int ncols int begin_y int begin_x)
)
(defn nc.mvderwin (win par_y par_x)
  (nc.call "mvderwin" int ptr win int par_y int par_x)
)
(defn nc.dupwin (win)
  (nc.call "dupwin" ptr ptr win)
)
(defn nc.wsyncup (win)
  (nc.call "wsyncup" void ptr win)
)
(defn nc.syncok (win bf)
  (nc.call "syncok" int ptr win int (if bf 1 0))
)
(defn nc.wcursyncup (win)
  (nc.call "wcursyncup" void ptr win)
)
(defn nc.wsyncdown (win)
  (nc.call "wsyncdown" void ptr win)
)

(defn nc.border (ls rs ts bs tl tr bl br)
  (nc.call "border" int chtype ls chtype rs chtype ts chtype bs chtype tl chtype tr chtype bl chtype br)
)
(defn nc.wborder (win ls rs ts bs tl tr bl br)
  (nc.call "wborder" int ptr win chtype ls chtype rs chtype ts chtype bs chtype tl chtype tr chtype bl chtype br)
)

(defn nc.box (win verch horch)
  (nc.call "box" int ptr win chtype verch chtype horch)
)

(def .ncurses.stdscr (nc.var "stdscr"))
(defn nc.stdscr () (read-var ptr .ncurses.stdscr))
(def .ncurses.LINES (nc.var "LINES"))
(defn nc.LINES () (read-var int .ncurses.LINES))
(def .ncurses.COLS (nc.var "COLS"))
(defn nc.COLS () (read-var int .ncurses.COLS))
